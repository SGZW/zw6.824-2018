...
func TestReElection2A(t *testing.T) {
	...
	DShortPrintf(-1, "start pass disconnect leader1")
	cfg.disconnect(leader1)
	cfg.checkOneLeader()
	DShortPrintf(-1, "end pass disconnect leader1")

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	DShortPrintf(-1, "start pass join leader1")
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	DShortPrintf(-1, "end pass join leader1")
	...
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	DShortPrintf(-1, "check no leader")

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
	DShortPrintf(-1, "check one leader last 1")

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	cfg.checkOneLeader()
	DShortPrintf(-1, "check one leader last 2")
